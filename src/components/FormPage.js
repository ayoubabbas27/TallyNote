...
export default function FormPage({ setPage , setSelectedNote }) {
  ...
  function handleAddingNote () {
    if ((noteTitle.trim().replace(/\s+/g, '') !== '') && (noteTags.trim().replace(/\s+/g, '') !== '') && isContentNotEmpty(noteContent)){
      ...
      const noteObj = {
        id: noteId,
        title: noteTitle,
        tags: noteTags.trim().replace(/\s+/g, '').split(',').filter(tag => (tag !== '' && tag !== '#')),
        date: noteDate,
        content: noteContent,
        pinned: false
      }
      chrome.runtime.sendMessage({ type: 'addNote' , data: noteObj }, (response) => {
        if (response.success){
    ...
    <div className='w-full h-full p-4 flex flex-col justify-center items-center gap-4 text-base'>
      ...
      <div className='w-full h-full flex flex-col justify-center items-center gap-3'>
        ...
        <div className='w-full h-full flex flex-col justify-center items-start'>
          ...
          <RichTextEditor noteContent={noteContent} setNoteContent={setNoteContent} selectedNote={null} />
        </div> 
      </div>
    </div>
  )
